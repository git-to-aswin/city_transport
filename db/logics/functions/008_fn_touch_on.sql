CREATE OR REPLACE FUNCTION journey.fn_touch_on(
  p_card_id BIGINT,
...
BEGIN
  ...
  -- get row station id
  SELECT MIN(id) INTO v_station_row_id
  FROM ref.stations 
  ...
  WHERE rrs.station_id = v_station_row_id
  ORDER BY rrs.route_id
  LIMIT 1;

  IF v_route_id IS NULL THEN
    RAISE EXCEPTION 'No route for station_id %', p_station_id;
  END IF;
